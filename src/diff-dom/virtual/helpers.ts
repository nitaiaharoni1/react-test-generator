...
function findUniqueDescriptors(li: any) {
  const uniqueDescriptors: any = {}
  const duplicateDescriptors: any = {}
...
function uniqueInBoth(l1: any, l2: any) {
  const l1Unique: any = findUniqueDescriptors(l1)
  const l2Unique: any = findUniqueDescriptors(l2)
  const inBoth: any = {}
...
export function isEqual(e1: any, e2: any) {
  ...
  if (e1.childNodes) {
    ...
    if (!e1.childNodes.every((childNode: any, index: any) => isEqual(childNode, e2.childNodes[index]))) {
...
export function roughlyEqual(e1: any, e2: any, uniqueDescriptors: any, sameSiblings: any, preventRecursion?: any) {
  ...
  if (preventRecursion) {
    return nodeList1.every((element: any, index: any) => element.nodeName === nodeList2[index].nodeName)
  } else {
    // note: we only allow one level of recursion at any depth. If 'preventRecursion'
    // was not set, we must explicitly force it to true for child iterations.
    const childUniqueDescriptors = uniqueInBoth(nodeList1, nodeList2)
    return nodeList1.every((element: any, index: any) => roughlyEqual(element, nodeList2[index], childUniqueDescriptors, true, true))
  }
}

export function cloneObj(obj: any) { //  TODO: Do we really need to clone here? Is it not enough to just return the original object?
...
function findCommonSubsets(c1: any, c2: any, marked1: any, marked2: any) {
  ...
  const matches: any = Array(...new Array(c1Length + 1)).map(() => [])
